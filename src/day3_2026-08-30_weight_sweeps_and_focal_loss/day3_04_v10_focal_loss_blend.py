"""Insight 2.0 v10: focal-loss (gamma=1) LGBM + XGB + CatBoost, 10-fold x 2 seeds.
...
def main():
    ...
    for seed in SEEDS:
        ...
        for f, (i_tr, i_va) in enumerate(skf.split(X, y)):
            ...
            m = lgb.train(p, lgb.Dataset(X.iloc[i_tr], y[i_tr].astype(float)),
            ...
            dtr = xgb.DMatrix(Xi.iloc[i_tr], label=y[i_tr]); dva = xgb.DMatrix(Xi.iloc[i_va], label=y[i_va])
    ...
    for m in models:
        ...
        np.save(f"data/processed/oof_v10_{m}.npy", oof[m])
        np.save(f"data/processed/test_v10_{m}.npy", tep[m])
    ...
    np.save("data/processed/oof_v10_blend.npy", p_blend)
    np.save("data/processed/test_v10_blend.npy", p_test)
