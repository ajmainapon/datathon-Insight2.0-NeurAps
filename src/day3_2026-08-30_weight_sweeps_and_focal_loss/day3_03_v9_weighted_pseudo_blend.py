"""Insight 2.0 v9: v4 pseudo recipe + scale_pos_weight 0.6 + year sample weights.

Tests whether reweighting the loss toward the Alive minority and toward recent
(harder) diagnosis years survives blending. It did not (see experiments/log.md #21).

Usage: .venv/bin/python src/day3.../day3_03_v9_weighted_pseudo_blend.py <sub_number> <description> <lo> <hi>
"""
import sys
import os
...
def best_threshold(y, p, lo=0.25, hi=0.75, step=0.005):
...
def main():
    ...
    print(f"pseudo rows: {conf_mask.sum()}/{len(te)} (Dead {y_pseudo.mean():.3f}) lo={lo} hi={hi}",
          flush=True)
    ...
    yw_tr = 1.0 + YW_ALPHA * (tr["year_of_diagnosis"].values - 2013) / 10.0
    yw_ps = 1.0 + YW_ALPHA * (te["year_of_diagnosis"].values[conf_mask] - 2013) / 10.0
    ...
    for seed in SEEDS:
        ...
        for f, (i_tr, i_va) in enumerate(skf.split(X, y)):
            ...
            w_aug = np.concatenate([yw_tr[i_tr], yw_ps])
            ...
            xm = xgb.XGBClassifier(n_estimators=3000, learning_rate=0.05, max_depth=7,
                                   ...
                                   scale_pos_weight=0.6)
            xm.fit(Xi_aug, y_aug, sample_weight=w_aug, eval_set=[(Xi.iloc[i_va], y[i_va])], verbose=False)
            ...
            cm.fit(Xc_aug, y_aug, sample_weight=w_aug, eval_set=(Xc.iloc[i_va], y[i_va]), cat_features=cat_cols)
            oof["cat"][i_va] += cm.predict_proba(Xc.iloc[i_va])[:, 1] / len(SEEDS)
            tep["cat"] += cm.predict_proba(Xc_te)[:, 1] / (N_FOLDS * len(SEEDS))
        print(f"seed {seed} done", flush=True)

    tag = f"v9_{sub_num}"
    print(f"\n=== per-model OOF wF1 ({tag}) ===")
    for m in models:
        t, s = best_threshold(y, oof[m])
        print(f"{m}: wF1={s:.5f} @ t={t:.3f}")
        np.save(f"data/processed/oof_{tag}_{m}.npy", oof[m])
        np.save(f"data/processed/test_{tag}_{m}.npy", tep[m])
    ...
    np.save(f"data/processed/oof_{tag}_blend.npy", p_blend)
    np.save(f"data/processed/test_{tag}_blend.npy", p_test)
    ...
    print(f"global: wF1={s_global:.5f} @ t={t_global:.3f} | per-fold median t={t_robust:.3f} wF1={s_robust:.5f}")
