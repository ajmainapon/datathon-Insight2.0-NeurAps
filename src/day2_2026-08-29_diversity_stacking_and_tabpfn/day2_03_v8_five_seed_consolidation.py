"""Insight 2.0 v8: v4 pseudo recipe, 5-seed consolidation, lr 0.03 / 6000 rounds.

Same pseudo source as day1_05 (test_v4_blend.npy); more seeds and a lower learning rate
to check whether variance reduction alone (no new signal) improves the blend.

Usage: .venv/bin/python src/day2.../day2_03_v8_five_seed_consolidation.py <sub_number> <description> <lo> <hi>
"""
import sys
import os
...
N_FOLDS = 10
SEEDS = [42, 2026, 7, 555, 31]
LGB_PARAMS = dict(objective="binary", learning_rate=0.03, num_leaves=63, min_data_in_leaf=40,
...
def main():
    ...
    for seed in SEEDS:
        ...
        for f, (i_tr, i_va) in enumerate(skf.split(X, y)):
            ...
            m = lgb.train(p, lgb.Dataset(X_aug, y_aug), num_boost_round=6000,
            ...
            xm = xgb.XGBClassifier(n_estimators=6000, learning_rate=0.03, max_depth=7,
            ...
            cm = cb.CatBoostClassifier(iterations=6000, learning_rate=0.03, depth=7,
