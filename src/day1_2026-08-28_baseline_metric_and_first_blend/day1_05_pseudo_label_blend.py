"""Insight 2.0 v4: pseudo-labeling from v3 blend test predictions + LGBM/XGB/CatBoost blend.

Confident test rows (p > hi or p < lo from v3 blend) are added to each fold's TRAINING
data only; OOF is still computed on original train rows -> unbiased CV.

Usage: .venv/bin/python src/train_v4.py <sub_number> <description> [lo=0.02] [hi=0.99]
"""
import json
import os
import sys
...
def main():
    ...
    hi = float(sys.argv[4]) if len(sys.argv) > 4 else 0.99

    tr = pd.read_csv("data/raw/train.csv")
    te = pd.read_csv("data/raw/test.csv")
    y = (tr["vital_status"] == "Dead").astype(int).values

    p_test_v3 = np.load("data/processed/test_v3_blend.npy")
    conf_mask = (p_test_v3 > hi) | (p_test_v3 < lo)
    y_pseudo = (p_test_v3[conf_mask] > 0.5).astype(int)
    print(f"pseudo-labeled test rows: {conf_mask.sum()} of {len(te)} "
          f"(Dead {y_pseudo.mean():.3f}) [lo={lo}, hi={hi}]")
    ...
    for m in models:
        ...
        np.save(f"data/processed/oof_v4_{m}.npy", oof[m])
        np.save(f"data/processed/test_v4_{m}.npy", tep[m])
    ...
    np.save("data/processed/oof_v4_blend.npy", p_blend)
    np.save("data/processed/test_v4_blend.npy", p_test)
    ...
    print(f"per-fold median t={t_robust:.3f}, wF1={s_robust:.5f}")
